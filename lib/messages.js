...
var messages = {
    ...
    scriptTags: function (port, options) {
        ...
        if (options.localBroadcast) {
            template = fs.readFileSync(config.templates.localBroadcastScriptTag, "utf-8");
        } else {
            template = fs.readFileSync(config.templates.scriptTag, "utf-8");
        }
        ...
        if (options.localBroadcast) {
            script = script.replace("HOST", options.localBroadcastHost);
        }
    ...
    socketConnector: function (port, options, external) {
        ...
        if (options.localBroadcast) {
            namespace = "\"" + options.localBroadcastHost + ":" + options.port + "/browser-sync\"";
        }
